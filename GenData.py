...
###################################################################################################
def main():
    imgTrainingNumbers = cv2.imread("21.png")            # read in training numbers image
    # cv2.imshow("Original image", imgTrainingNumbers)
    # cv2.waitKey(0)
    # cv2.destroyAllWindows()

    scale_percent = 1300  # percent of original size
    width = int(imgTrainingNumbers.shape[1] * scale_percent / 100)
    height = int(imgTrainingNumbers.shape[0] * scale_percent / 100)
    dim = (width, height)
    # resize image
    imgTrainingNumbers = cv2.resize(imgTrainingNumbers, dim, interpolation=cv2.INTER_NEAREST)
    ...
    imgBlurred = cv2.GaussianBlur(imgGray, (7, 7), 0)                        # blur
