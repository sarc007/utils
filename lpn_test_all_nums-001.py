...
import numpy as np
import h5py
from global_objectives.loss_layers_example import LEARNING_RATE
...
data_file = dst_dir + "\\" + "alpha_nums.hdf5"
...
x_train, x_test, y_train, y_test = train_test_split(image_arr, labels_arr, test_size=0.3, random_state=21)
...
def create_model():
	...
	adam = optimizers.Adam(lr=0.0001, decay=1e-6, epsilon=0.00000001, beta_1=0.9, beta_2=0.999)
...
model.fit(x_train, y_train,  validation_data=(x_test, y_test), epochs=40,
          validation_split=0.3,  verbose=1, shuffle=True, callbacks=[cp_callback])
val_loss, val_acc = model.evaluate(x_test, y_test)
print("Loss = {0}' , Accuracy = {1} ", val_loss, val_acc)
model.save(model_dir + '\\reader_allnums.hdf5', overwrite=True)
new_model = tf.keras.models.load_model(model_dir + '\\reader_allnums.hdf5')
